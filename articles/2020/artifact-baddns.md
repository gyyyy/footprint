...
与上面一样依次给『结果处理』和『统计』发送消息后，再继续判断当前是否已经是最大深度 *（由`-l`参数决定，默认为`1`）* ，以及当前子域名是否在子域名字典中，即是否支持继续下一层的探测，全部条件都通过时，就扔给『域名生成』一个深度+1的`GenItem`，同时发送`Statue::TargetCount`给『统计』：
